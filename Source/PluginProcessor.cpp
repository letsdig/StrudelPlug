...
void StrudelPlugAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    //
    // midiMessages still holds the DAW's own incoming notes at this point
    // (we only read them in step 2, we never removed them). If we don't
    // clear the buffer first, those input notes get echoed straight back
    // out to the host mixed in with whatever Strudel actually generated -
    // so from Bitwig's side the plugin's "MIDI out" looks like a copy of
    // its own input rather than the browser's output, which is what made
    // it look like Bitwig wasn't picking up the plugin's generated MIDI.
    midiMessages.clear();
